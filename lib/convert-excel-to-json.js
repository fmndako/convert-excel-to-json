...
const excelToJson = (function() {
    ...
    function getSheetCellValue(sheetCell) {
        ...
        return (sheetCell.t === 'n' || sheetCell.t === 'd') ? getDateValue(sheetCell) : (sheetCell.w && sheetCell.w.trim && sheetCell.w.trim()) || sheetCell.w;
    };

    function getDateValue(sheetCell){
        if(!_config.inputDateFormat) return sheetCell.v;
        if (sheetCell.t === 'd' || (sheetCell.w && ( (sheetCell.v && !sheetCell.v.toString().includes('-')) && (sheetCell.w.includes('-') || sheetCell.w.includes('/'))))) {
            // let format = _config.inputDateFormat;
            let d = XLSX.SSF.parse_date_code(sheetCell.v);
            if(!d) return sheetCell.v;
            return new Date(d.y, d.d - 1, d.m);
            // if (format.indexOf('d') > format.indexOf('m')) return new Date(d.y, d.d - 1, d.m);
            // else return new Date(d.y, d.m - 1, d.d);
    ...
    const convertExcelToJson = function(config = {}, sourceFile) {
        ...
        // If inputDateFormat is supplied, date conversion is as described n format,
        _config.inputDateFormat = _config.inputDateFormat || false;
        let cellDates = _config.inputDateFormat ? false : true;
